...
# get supercategories
with open("labels/superclass.txt", "r") as sc:
    supercat_id = [s.strip() for s in sc.readlines()]
...
with torch.no_grad():
    for batch in dataloader:
        ...
        for output, i in zip(outputs, y):
            ...
            # Check whether the actual label is in the top category per image
            top_prob, top_catid = torch.topk(probabilities, 1)
            
            if top_catid == int(classes[i.item()]):
                accuracy[classes[i.item()]].append(1)
                probs[classes[i.item()]].append(top_prob.item())
            elif supercat_id[top_catid] == supercat_id[int(classes[i.item()])]:
                accuracy[classes[i.item()]].append(1)
                probs[classes[i.item()]].append(top_prob.item())
            else:
...
with open('probs_superclass.csv', 'w') as csvfile:
...
with open('accuracies_superclass.csv', 'w') as csvfile:
    writer = csv.writer(csvfile)
